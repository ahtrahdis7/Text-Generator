...
# from static.shakespeare_utils import *
import io
import sys
...
def processing(text):
    ...
    X, Y = build_data(text, Tx, stride = 3)
    print("Vectorizing training set...")
    x, y = vectorization(X, Y, n_x = 38, char_indices = char_indices) 
    # print("Loading model...")
...
def generate_output(usr_input, Tx):
    global chars
    generated = ''
    #sentence = text[start_index: start_index + Tx]
    #sentence = '0'*Tx
...
def get_input_text():
    input = request.args.get('input_text')
    # global chars
    processing(input)
    return {"return":"training..."}
